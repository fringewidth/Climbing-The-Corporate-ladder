...
export default class Experience {
  constructor(canvas) {
    ...
    var pc2;
    var pc3;
    loader.load("./gltfs/test_optimised.glb", function (gltf) {
      cube = gltf.scene;
    ...
    loader.load("./gltfs/lvl1.glb", function (gltf) {
      pc1 = gltf.scene;
      gltf.scene.traverse(function (node) {
        if (node.isMesh) {
          node.castShadow = true;
    ...
    loader.load("./gltfs/lvl2.glb", function (gltf) {
      pc2 = gltf.scene;
      gltf.scene.traverse(function (node) {
        if (node.isMesh) {
          node.castShadow = true;
    ...
    loader.load("./gltfs/lvl3.glb", function (gltf) {
      pc3 = gltf.scene;
      gltf.scene.traverse(function (node) {
        if (node.isMesh) {
          node.castShadow = true;
    ...
    function animate() {
      ...
      console.log(zloc);
      const linkData = [
        { zMin: 25.5, zMax: Infinity, text: "", href: "" },
        {
          zMin: 20.5,
          zMax: 25.5,
          text: "Dhyaan and Hrishik Present",
          href: "",
        },
        {
          zMin: 11.5,
        ...
        },
        {
          ...
          zMax: 11.5,
          text: "Play Level 1",
          href: "Level-1/level-1.html",
        },
        {
          zMin: 2.5,
          zMax: 3.5,
          text: "Play Level 2",
          href: "Level-2/level-2.html",
        },
        {
          zMin: -Infinity,
          zMax: -3.5,
          text: "Play Level 3",
          href: "Level-3/level-3.html",
        },
      ];

      for (const data of linkData) {
        if (zloc >= data.zMin && zloc < data.zMax) {
